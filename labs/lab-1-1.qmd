...
In this lab we will get introduced to geographical information, how to process, analyze, and visualize it. In particular
...
-   Of course, `R` can be used also as a GIS.
    ...
    -   There are mainly two categories:
        ...
        -   Libraries for visualization like `leaflet`, `mapview`.
...
**Raster data** is a geographical data type where data is stored as a grid of regularly sized pixels along attribute data. Those pixels typically represent a grid cell in specific geographical coordinates.
...
-   The most used format for raster data is GeoTIFF.
...
**Basemaps** a particular case of raster images is images of maps used as a background display for other feature layers.

-   Main source of basemaps are satellite images, aerial photography, scanned maps, or composed maps. For example, the maps you see in [Google Maps](https://maps.google.com), \[Open Street Maps\], \[Carto\], \[MapBox\].
...
**Vector data** is a representation of data using points, lines, curves, or polygons. The data is created by defining points at specific spatial coordinates. For example, lines that define a route on a map, polygons that define an administrative area, or points that geolocate a specific store entrance.

![Vector data - Roads around Boston](./img/vectordata.png)

-   Vector data is stored and share in many different formats. The most used is the ESRI Shapefile (.shp), GeoJSON, KML (Keyhole Markup Language).

-   The Shapefile format, which consists on multiple files (e.g., .shp, .shx, .dbf) is one of the oldest and most widely recognized standards, supported by most GIS software and libraries in `R` and `python`.

-   GeoJSON is a text-based format derived from JSON, making it easy to integrate in web mapping libraries and APIs.

## Libraries to work with raster data

-   **Raster data**. The most prominent and powerful library to work with raster data in `R` is [`terra`](https://rspatial.github.io/terra/). It can also work with vector data, but his main strength is raster data.

-   Let's see an example. [WorldPop](https://hub.worldpop.org) is an organization that provides spatial high-resolution demographic datasets, like population. It is widely used specially in areas where the census data is not very good. Estimations of population are done using official data, satellite images, and other data.

-   Get the raster for the estimated total number of people at a resolution of 30 arc seconds (approximatelly 1km at the equator). Link [here](https://hub.worldpop.org/geodata/summary?id=34879)
...
The Atmostpheric Composition Analysis Group at WashU publishes different measures of pollution at one of the smallest resolutions available (0.01º $\times$ 0.01º) using different chemical, atmospheric models together with satellite image and ground datasets (see @van2024north).

-   Navigate to the webpage from the group [WashU Satellite-derived PM2.5](https://sites.wustl.edu/acag/datasets/surface-pm2-5/)
-   Find the Monthly mean total component PM$_{2.5}$ at 0.01º $\times$ 0.01º in the US in NetCDF format. Use the V5.NA.04.02 version.
-   Download the data for the months of February, March and April 2020. Do you see any difference?
...
In `R` the library [`sf`](https://r-spatial.github.io/sf/) is the most widely adopted package for handling, manipulating, and visualizing vector data. It offers a modern, "tidyverse-friendly" interface, making it straightforward to integrate spatial data operations within standard R workflows.
...
Let's see an example. The U.S. Census Bureau provides county boundaries in a zipped shapefile format. We’ll download that file, unzip it to a temporary directory, and then read it into R as an sf object.

-   Download the 20m resolution of county boundaries for 2020 on a temporal directory and unzip in the data directory
...
-   The `sf` object in `counties` looks like a table. Each row contains a *feature* that can be a point, line, polygon, multi-line or multipolygon.
    -   The geometry (vector) of each feature is in the `geometry` column.
    -   The rest of the columns are metadata for each of the features. In our case, we have the name of the county, the State where the county is, or the U.S. Census internal ID (`GEOID`) and many other.
-   We can work with `sf` objects like with any other table in `R`. For example, here is the map of the counties in mainland US

```{r}
counties |> filter(!(STUSPS %in% c("AK","PR","HI"))) |> ggplot() + geom_sf()
```

-   Another way to get these vector data from the census is the amazing [`tidycensus` library](https://walker-data.com/tidycensus/) which not only give us the geometry of each county but also some demographic information about them. For example, let's get the median household income and population by county and their geometry. Note the `geometry = TRUE` at the end to get the shape of the counties.
...
counties_income <- get_acs(geography = "county", 
                           ...
                           progress_bar = FALSE)
```
We will come back to the use of `tidycensus` in the next labs.

## Exercise 2

Another source of vector data is Open Street Map (OSM). OSM is a collection of APIs where we can download roads, Points-of-Interests, buildings polygons, routes, and many more. In `R` we can download data from OSM using the fantastic `osmdata` package.
...
-   OSM contains many layers (look at their maps!), like rivers, highways, Points-of-interest, etc. You can find a comprehensive list of them [here](https://wiki.openstreetmap.org/wiki/Map_features) or by using

```{r}
available_features() |> head()
```

-   Let’s see how many of them are related to highway.
...
Finally, the other type of important of geographical information are basemaps. Basemaps are usual in raster or image format and compile information about geographical area as a picture. Main source of basemaps are satellite images, aerial photography, scanned maps, or composed maps. For example, the maps you see in [Google Maps](https://maps.google.com).

-   There are many APIs to retrieve those maps, including \[Google Maps\], [OSM](https://www.openstreetmap.org/), [Stadia Maps](https://stadiamaps.com), [Carto](https://carto.com), [Mapbox](https://www.mapbox.com), and many others. Those APIs return *tiles* which are static images of those maps at different zoom (resolutions). Most of those services require an API Key.

-   There are many libraries to download those basemaps/tiles from different APIs, like the library `ggmap`. However, since we typically will need basemaps to show data on top we are going to show a more dynamical way to access them through libraries like `leaflet` or `mapview`. Also those libraries do not require to set a API key.

-   In those libraries, corresponding basemaps will be automatically download from one of those APIs to match our geographical data.

-   For example, let's visualize our counties data set on top of a basemap. Let's start with the library `leaflet`, which is also "tidyverse-friendly", but without basemap. Note that only the polygons are shown
...
-   Also note that the map is interactive: you can zoom in, move around. Let's add the basemap by default (OSM):
...
-   Those libraries allow to visualize many different layers on top of each other. We will come back to these libraries and their visualizations options at the end of the lab.
...
Projections are defined by the Coordinate Reference Systems (CRS) which specify how latitute and longitude relate to a particular mathematical model of the Earth. And yes, there are many mathematical models of the Earth, because it is not a sphere, but rather a ellipsoid
...
-   The [EPSG Geodetic Parameter Dataset](https://en.wikipedia.org/wiki/EPSG_Geodetic_Parameter_Dataset) contains a registry of thos spatial reference systems, earth ellipsoids. Funny story, it was initiated by the European Petroleum Survey Group (EPSG) in 1985. Each coordinate system has a number (4326, 3857).

-   **Important**: Using different map projections can produce varying coordinate values for the same location on Earth. Thus, it is important to know what is the projection of our data. Most errors in using geographical information comes from using different geographical projections.

-   We can use `sf` library to get the geographical projection in which our datasets are given. For example, `counties` from the U.S. Census comes in the NAD83 format
...
-   Since the `sf` objects are tables, we can use `R` and `tidyverse` to work with them. To exemplify, let's get all the McDonald's locations in the US using the recently released [Foursquare Open Source Places](https://location.foursquare.com/resources/blog/products/foursquare-open-source-places-a-new-foundational-dataset-for-the-geospatial-community/). This is a huge dataset of 100mm+ global places of interest.
...
-   We select only McDonald's in the US and exclude those without location
...
-   For example, how many McDonald's do we have by county? To do that we need to intersect our dataset of POIs with the polygons for each county. For that we can use a number of geographical *join* functions, with different predicates `st_within`, `st_intersects`, `st_overlaps` and many others. We can also use the typical subsetting `[` function in `R`. Find more information in the `sf` [spatial join, spatial filter reference webpage](https://r-spatial.github.io/sf/reference/st_join.html).
...
Objective: Investigate whether there is an economic bias in where McDonald’s locations are situated and identify which communities (by income) they predominantly serve.

-   Use `st_join` to indenty the county for each of the McDonald's in the US.

-   Spatial join of the points and counties
...
-   Count the number of McDonald's by county. Note that we drop the geometry before to speed up the calculation
...
- Use other fast-food restaurants for this analysis. Do you see the same correlation?
...
-   We can calculate distances between points, lines, polygons using `st_distance`. For example, what is the distance between all McDonald's in the Middlesex county?
...
Which means we have, in median, one McDonald's each 2.6km in the Middlesex county.

-   Other operations involve calculating areas of polygons with `st_area`. For example, here are the largests counties in the US.
...
-   Another transformation is to find the centroids of geographical features. This is useful to calculate distances, because distances between points is much less costly than with lines or polygons.
...
We saw before some libraries to visualize geographical information: `ggplot` to produce static visualizations and `leaflet` and `mapview` for interactive visualizations with automatic basemaps. The latter ones typically are better but they require more computational resources, so they are slow for big data. map

There are many options in those libraries to customize our visualization. For example, we can change the colors, add different layers, legends, etc. Here we are going to exemplify this by creating a `choropleth` visualization. A choropleth is an easy way to visualize how a variable varies across geographical area by changing the color or transparency of those geographical areas. You can see them everywhere, in newspapers, reports. Here are the 2016 elections by district

![2016 Election results by district, © New York Times](./img/2016elections.jpg) Caution should be taken when using this kind of visualization. Choropleths are prone to misinterpretation when areas are not uniform in size (the "Alaska effect") given more importance to larger areas which typically are less populated (see a good example above!).
